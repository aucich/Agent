function render(buffer) {
  ...
  const keyLink = (label, c) => {
    const t = encodeURIComponent(buffer);
    const cc = encodeURIComponent(c);
    return `<a style="display:inline-block;padding:8px 10px;margin:4px;border:1px solid #999;border-radius:8px;text-decoration:none" href="/k?t=${t}&c=${cc}">${label}</a>`;
  };
...
export async function onRequestGet({ request }) {
  ...
  // URLSearchParams.get() restituisce già una stringa decodificata
  let buf = url.searchParams.get("t") || "";
